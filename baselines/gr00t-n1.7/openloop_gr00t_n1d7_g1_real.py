#!/usr/bin/env python3
"""Open-loop GR00T-N1.7 inference on a SONIC lerobot_v2.1 episode (33D/68D).

Same rollout cadence as ACT / Psi0 open-loop:

  1. At frame ``t``, query with GT stereo images[t] + state[t].
  2. Write ``chunk[0 : n_action_steps]`` to frames ``t .. t+n_action_steps-1``.
  3. Advance ``t += n_action_steps`` and repeat.

Requires a running GR00T ZMQ PolicyServer::

    bash baselines/gr00t-n1.7/serve_gr00t_n1d7_sonic.sh \\
      --model-path .../checkpoint-40000

Output::

    <data_root>/openloop_gr00t_n1d7_{ckpt_step}/episode_{id:06d}/
      pred_actions.npy
...
from gr00t_zmq_client import ACTION_DIM, HAND_DIM, STATE_DIM, TOKEN_DIM, Gr00tZMQClient

IMAGE_KEY_LEFT = "observation.images.egocentric_left"
...
def resolve_openloop_dir(data_root: Path, ckpt_step: int, episode_idx: int) -> Path:
    return (
        Path(data_root)
        / f"openloop_gr00t_n1d7_{int(ckpt_step)}"
...
def plot_pred_vs_gt(
    ...
    for dim, ax in enumerate(axes):
        ...
        if dim == 0:
            ...
            ax.set_title(
                f"GR00T-N1.7 open-loop ep{episode_idx}  |  all {ACTION_DIM} dims", fontsize=11
            )
...
def rollout_pred_actions(
    policy: Gr00tZMQClient,
    video_left: VideoReader,
...
def parse_args():
    p = argparse.ArgumentParser(description="GR00T-N1.7 SONIC open-loop → pred_actions.npy")
    p.add_argument(
        "--data-root",
        type=Path,
        required=True,
        help="Task dataset root; writes openloop_gr00t_n1d7_{ckpt}/episode_XXXXXX/",
    )
    ...
    p.add_argument("--port", type=int, default=5555, help="GR00T PolicyServer ZMQ port")
    p.add_argument(
        "--n-action-steps",
        type=int,
        default=1,
...
def main() -> None:
    ...
    video_left = VideoReader(resolve_episode_video(dataset_dir, args.episode_idx, info, IMAGE_KEY_LEFT))
    video_right = VideoReader(
        resolve_episode_video(dataset_dir, args.episode_idx, info, IMAGE_KEY_RIGHT)
    )
    print(f"[DATA] left={video_left.path}")
    print(f"[DATA] right={video_right.path}")

    policy = Gr00tZMQClient(host=args.host, port=args.port)
    print(f"[GR00T] Checking server {args.host}:{args.port} ...")
    if not policy.ping():
        print("[GR00T] server not healthy. Start baselines/gr00t-n1.7/serve_gr00t_n1d7_sonic.sh first.")
        sys.exit(1)
    try:
        policy.reset()
    except Exception as exc:
        print(f"[GR00T] reset warning: {exc}")
