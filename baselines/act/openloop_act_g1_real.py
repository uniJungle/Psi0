#!/usr/bin/env python3
"""Open-loop ACT inference on a Psi0 SONIC lerobot_v2.1 episode.

Rollout (matches real ``act_inference`` replan cadence):

  1. At frame ``t``, query ACT with **GT** image[t] + state[t].
  2. Write ``chunk[0 : n_action_steps]`` to frames ``t .. t+n_action_steps-1``.
  3. Advance ``t += n_action_steps`` and repeat until the episode ends.

Observations always come from the dataset (not simulated from pred actions).
``--n-action-steps 1`` = replan every frame (only chunk[0] each time).
        ...
        rows.append(
            {
                ...
                "action": act.tolist(),
                # Extra columns so replay_real token mode works without guessing.
...
def write_meta(
    ...
    # Keep psi core features; annotate extra replay columns.
    ...
    acts = None  # filled by caller via episodes_stats if needed
    _ = acts
...
def plot_pred_vs_gt(
    ...
    """Save one figure: 68 rows × 1 col, each row one action dim (pred + GT)."""
    ...
    for dim, ax in enumerate(axes):
        ...
        if dim == 0:
            ...
            ax.set_title(f"ACT open-loop ep{episode_idx}  |  all {ACTION_DIM} dims", fontsize=11)
...
def parse_args():
    ...
    p.add_argument(
        ...
        "(same replan cadence as real act_inference). 1 = every frame uses chunk[0] only.",
    )
    p.add_argument(
        "--max-frames",
        type=int,
...
def rollout_pred_actions(
    ...
    """Chunk rollout: GT obs at t → write chunk[0:steps] → next query at t+steps."""
...
def main():
    ...
    print(f"[ACT] Checking server {args.host}:{args.port} ...")
    if not policy.health_check():
        print("[ACT] server not healthy. Start baselines/act/serve_act_g1_real.sh first.")
        sys.exit(1)
    policy.reset()
    print(
        f"[ROLLOUT] n_action_steps={max(1, args.n_action_steps)} "
        f"(server must return chunk length >= this; set serve --n-action-steps)"
    )
