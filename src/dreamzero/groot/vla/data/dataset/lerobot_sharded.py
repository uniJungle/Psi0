...
from dreamzero.groot.vla.common.utils import get_frames_by_timestamps
from dreamzero.groot.vla.data.conversion.g1_simple.action_padding import freeze_action_36d
...
class ShardedLeRobotSubLangSingleActionChunkDatasetDROID(LeRobotSingleDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # End-of-episode padding (psi0_simple-only). Set by the parent
        # ShardedLeRobotMixtureDataset after construction; default off so
        # standalone use of this class is unchanged.
        self.allow_padding_at_end: bool = False
        ...
        if language_key is not None and language_key in traj_data.columns and len(step_indices) > 0:
            ...
            def add_anchor(anchor_index: int) -> None:
                ...
                if target_num_chunks is not None and len(sampled_list) >= target_num_chunks:
                    return
                # Match video's add_step_set boundary so chunk counts agree
                # across video / state / action. State delta is [0]: when
                # allow_padding_at_end is set, the only requirement is that
                # the anchor itself is in-bounds (state never needs padding).
                anchor_upper = (
                    trajectory_length
                    if self.allow_padding_at_end
                    else trajectory_length - self.action_steps_per_chunk + 1
                )
                if 0 <= anchor_index < anchor_upper:
                    sampled_list.append(int(anchor_index))

            # Always include first_idx anchor
            add_anchor(first_idx)

            # Expand outward in chunk-sized steps
            chunk_stride = self.action_steps_per_chunk
            step = 1
            back_done = True  # disabled: backward expansion oversamples mid-demo and locks L0 to first_idx mod aspc
            fwd_done = False
            while len(sampled_list) < max_frames and (not back_done or not fwd_done):
                ...
                if not back_done:
                    back_anchor = first_idx - chunk_stride * step
                    if back_anchor < 0:
                        back_done = True
                ...
                if len(sampled_list) >= max_frames:
                    break
                if not fwd_done:
                    fwd_anchor = first_idx + chunk_stride * step
    ...
    def get_action(
        ...
        if language_key is not None and language_key in traj_data.columns and len(step_indices) > 0:
            ...
            per_step_offsets = list(range(self.action_steps_per_chunk))
            sampled_list: list[int] = []

            def add_step_set(anchor_index: int) -> None:
                nonlocal sampled_list
                # Match the video path's boundary so chunk counts agree at
                # the trajectory tail. When allow_padding_at_end is set
                # (psi0_simple), accept anchors whose chunk overshoots
                # traj_len; the overshooting action rows are filled with
                # freeze_action_36d below.
                anchor_upper = (
                    trajectory_length
                    if self.allow_padding_at_end
                    else trajectory_length - self.action_steps_per_chunk + 1
                )
                if anchor_index < 0 or anchor_index >= anchor_upper:
                    return
                # Ensure we don't overrun the max_frames cap with a partial chunk
                if len(sampled_list) + self.action_steps_per_chunk > max_frames:
                    return
                # If we have a target number of chunks, stop when we reach it
                if target_num_chunks is not None and len(sampled_list) // self.action_steps_per_chunk >= target_num_chunks:
                    return
            ...
            while len(sampled_list) < max_frames and (not back_done or not fwd_done):
                # Stop if we've reached the target number of chunks
                if target_num_chunks is not None and len(sampled_list) // chunk_stride >= target_num_chunks:
                    break
            ...
            if len(sampled_list) > 0:
                ...
                divisible_size = (capped_size // self.action_steps_per_chunk) * self.action_steps_per_chunk
        ...
        # End-of-episode freeze padding (psi0_simple-only). When
        # allow_padding_at_end lets the last chunk overshoot traj_len, the
        # default "first_last" / "zero" strategy in retrieve_data_and_pad is
        # wrong: it would either replay the last commanded vx/vy or leave
        # absolute joint targets at zero, both of which AMO does NOT hold
        # still on. Replace those overshooting rows with freeze_action_36d
        # of the last real action so the policy is supervised to keep the
        # robot at its end pose.
        action_subkey = key.replace("action.", "")
        if (
            self.allow_padding_at_end
            and action_subkey == "joint_position"
            and action_data.shape[-1] == 36
            and len(sampled_indices) > 0
        ):
            pad_mask = sampled_indices >= max_length
            if pad_mask.any():
                frozen = freeze_action_36d(data_array[-1])
                action_data[pad_mask] = frozen
    ...
    def _uniform_sample_from_language_ranges(
        ...
        per_step_offsets = [i * video_stride for i in range(video_frames_per_chunk)]
        sampled_list: list[int] = []
        ...
        def add_step_set(anchor_index: int) -> None:
            ...
            # When allow_padding_at_end is set (psi0_simple), we relax the gate so
            # the first chunk's tail can overshoot traj_len; the overshooting frame
            # indices are clamped to the last real frame downstream in get_video.
            nonlocal sampled_list
            anchor_upper = (
                trajectory_length
                if self.allow_padding_at_end
        ...
        # Expand outward in both directions in chunk-sized steps
        chunk_stride = self.action_steps_per_chunk
        step = 1
        back_done = True  # disabled: backward expansion oversamples mid-demo and locks L0 to first_idx mod aspc
        fwd_done = False
        ...
        # unique_sorted should always has n * 8 frames now as "add_step_set" only adds a full set of 8 frames
        ...
        # Convert to 8n+1 format by adding one more frame at video_stride to maintain uniform spacing (8n → 8n+1)
        if unique_sorted.size > 0:
            last_idx = unique_sorted[-1]
            additional_idx = last_idx + video_stride

            # When allow_padding_at_end is set, the appended frame may
            # overshoot trajectory_length; get_video clamps it to
            # trajectory_length - 1, producing a frozen last frame
            # (visual analogue of freeze_action_36d for actions). This is
            # what unblocks tail anchors in psi0_simple — without it the
            # whole tail-anchor sample silently drops in get_step_data.
            can_append = unique_sorted.size < max_frames and (
                additional_idx < trajectory_length or self.allow_padding_at_end
            )
            if can_append:
                unique_sorted = np.append(unique_sorted, additional_idx)
            else:
                # Strict mode hit the trajectory boundary. Trim to 8n+1 by
                # dropping the last 7. Require at least 9 frames so
                # (noisy_frames - 1) // num_frame_per_block >= 1 for the
                # CausalWanModel invariant; otherwise return empty so the
                # sample is skipped.
...
class ShardedLeRobotMixtureDataset(LeRobotMixtureDataset, IterableDataset):
    ...
    def __init__(
        ...
        # Propagate end-of-episode padding flag down to children that support
        # it (currently only the DROID-style class used by psi0_simple).
        for dataset in self.datasets:
            if hasattr(dataset, "allow_padding_at_end"):
                dataset.allow_padding_at_end = allow_padding_at_end
        # Set properties
        ...
        for i, (dataset_index, shard_index) in enumerate(self.shards_sample_schedule):
            ...
            for trajectory_id in dataset.get_trajectories_in_shard():
                ...
                if self.allow_padding_at_end:
                    ...
                    allowed_length = trajectory_length - max_delta_index - 1
