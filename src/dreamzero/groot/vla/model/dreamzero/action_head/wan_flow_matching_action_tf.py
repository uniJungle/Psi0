...
from dreamzero.groot.vla.model.n1_5.action_head.base_action_head import ActionHead
from dreamzero.groot.vla.model.dreamzero.modules.flow_match_scheduler import FlowMatchScheduler
from dreamzero.groot.vla.model.dreamzero.modules.vram_management import enable_vram_management, AutoWrappedModule, AutoWrappedLinear
from dreamzero.groot.vla.model.dreamzero.modules.wan_video_text_encoder import T5RelativeEmbedding, T5LayerNorm
from dreamzero.groot.vla.model.dreamzero.modules.flow_unipc_multistep_scheduler import FlowUniPCMultistepScheduler
...
class WANPolicyHead(ActionHead):
    ...
    # def load_prompt_cache(self, cache_path: str):
    #     """Load precomputed prompt embeddings from a .pt file.

    #     Once loaded, encode_prompt() returns cached embeddings for known
    #     prompts and only falls back to the text encoder on cache miss.

    #     Args:
    #         cache_path: Path to a .pt file mapping tuple(input_ids) → prompt_emb.
    #     """
    #     self._prompt_emb_cache = torch.load(
    #         cache_path, map_location="cpu", weights_only=True,
    #     )
    #     print(f"[WANPolicyHead] Loaded {len(self._prompt_emb_cache)} "
    #           f"cached prompt embeddings from {cache_path}")

    def set_trainable_parameters(self, tune_projector: bool, tune_diffusion_model: bool):
        self.tune_projector = tune_projector # text_encoder, image_encoder, and vae
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        with torch.amp.autocast(dtype=torch.bfloat16, device_type=torch.device(self._device).type):
            ...
            if actions.numel() > 0:
                ...
                # has_real_action is a per-sample scalar, collated to shape (B,).
                # Broadcast it over the batch dim only; the original `[:, None]`
                # assumed action_loss_per_sample was 2D, which only holds at bs=1.
                _hr = has_real_action.float()
                while _hr.dim() < action_loss_per_sample.dim():
                    _hr = _hr.unsqueeze(-1)
                action_loss_per_sample = _hr * action_loss_per_sample  # apply has_real_action
    ...
    def lazy_joint_video_action(self, backbone_output: BatchFeature, action_input: BatchFeature, latent_video: torch.Tensor | None = None) -> BatchFeature:
        ...
        for index, current_timestep in enumerate(sample_scheduler.timesteps):
            ...
            if should_run_model:
                ...
                if self.cfg_scale != 1.0:
                    # Classifier-free guidance (needs the negative-prompt forward pass).
                    ...
                    # CFG disabled: single conditional forward, no guidance overshoot.
