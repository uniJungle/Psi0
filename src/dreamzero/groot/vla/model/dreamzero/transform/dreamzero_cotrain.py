...
from dreamzero.groot.vla.data.transform.base import InvertibleModalityTransform
from dreamzero.groot.vla.model.dreamzero.transform.common import formalize_language
        ...
        if key == "text":
            ...
            for elem in features:
                ...
                try:
                    ...
                    elif elem["embodiment_id"] == embodiment_tag_mapping[EmbodimentTag.SIMPLE.value]:
                        processed_item = "A single egocentric view video shows that a humanoid robot " + processed_item.lower()
                    elif elem["embodiment_id"] == embodiment_tag_mapping[EmbodimentTag.PSIX_G1_SONIC_NECK.value]:
                        processed_item = "A single egocentric view video shows that a humanoid robot " + processed_item.lower()
                    elif elem["embodiment_id"] == embodiment_tag_mapping[EmbodimentTag.G1_SONIC.value]:
                        processed_item = "A single egocentric view video shows that a humanoid robot " + processed_item.lower()
                    else:
                    ...
                    if num_views > 1 and elem["embodiment_id"] == embodiment_tag_mapping[EmbodimentTag.AGIBOT.value]:
                        ...
                        item = "A single egocentric view video shows that a humanoid robot " + str(item).lower()
                    elif elem["embodiment_id"] == embodiment_tag_mapping[EmbodimentTag.PSIX_G1_SONIC_NECK.value]:
                        item = "A single egocentric view video shows that a humanoid robot " + str(item).lower()
                    elif elem["embodiment_id"] == embodiment_tag_mapping[EmbodimentTag.G1_SONIC.value]:
                        item = "A single egocentric view video shows that a humanoid robot " + str(item).lower()
...
class DreamTransform(InvertibleModalityTransform):
    ...
    def apply_batch(self, data: dict, batch_size: int) -> dict:
        ...
        def _split_i(x, i):
            # 0-d scalar fields (e.g. a single language string when the dataset
            # has no paraphrases) have no batch/chunk dim, so broadcast them to
            # every split instead of indexing (x[i] would raise
            # "invalid index to scalar variable"). Fields with a batch dim are
            # indexed as before.
            ...
            try:
                ...
                return x
        data_split = [tree.map_structure(lambda x, _i=i: _split_i(x, _i), data) for i in range(batch_size)]
