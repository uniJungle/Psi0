...
from dreamzero.groot.vla.model.dreamzero.modules.wan2_1_attention import AttentionModule
from dreamzero.groot.vla.model.n1_5.modules.action_encoder import (
    SinusoidalPositionalEncoding,
    swish,
)
from dreamzero.groot.vla.model.dreamzero.modules.wan2_1_submodule import (
    WanRMSNorm,
    rope_action_apply,
    WanLayerNorm,
...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            if action_register_length is not None:
                # Match TRAINING: state tokens do SELF-ATTENTION ONLY
                # (_process_state_blocks); they must NOT attend the full context the
                # way action tokens do (issue #61). The register tail is laid out as
                # [action(na), state(ns)] for the single inference block
                # (action_register_length == na + ns), so split the state off.
                na = self.num_action_per_block
                q_act, q_st = roped_action_query[:, :na], roped_action_query[:, na:]
                k_act, k_st = roped_action_key[:, :na], roped_action_key[:, na:]
                v_act, v_st = action_v[:, :na], action_v[:, na:]
                # image + action: attend cached frames + current image + action + state
                # (unchanged from before — image/action outputs are identical).
                x_main = self.attn(
                    torch.cat([roped_query, q_act], dim=1),
                    torch.cat([new_k, k_act, k_st], dim=1),
                    torch.cat([new_v, v_act, v_st], dim=1),
                )
                # state: self-attention only, mirroring training's _process_state_blocks
                x_st = self.attn(q_st, k_st, v_st)
                x = torch.cat([x_main, x_st], dim=1)
            else:
                x = self.attn(
                    roped_query,
                    new_k,
