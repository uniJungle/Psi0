...
from dreamzero.groot.vla.data.schema import DatasetMetadata, EmbodimentTag
from dreamzero.groot.vla.data.transform import ComposedModalityTransform


def _replace_groot_prefix(module_path: str) -> str:
    if module_path.startswith("groot."):
        return f"dreamzero.{module_path}"
    return module_path
...
class GrootSimPolicy(BaseGrootSimPolicy):
    def __init__(
        ...
        Args:
            ...
            pretrained_base_path (str | None): Path to the pretrained base checkpoint
                (e.g. DreamZero-AgiBot) that the LoRA adapter was trained on top of.
                Required for correct LoRA inference when save_lora_only=True and the
                LoRA was fine-tuned from a pretrained base other than the raw backbone.
        """
        self._pretrained_base_path = pretrained_base_path
        ...
        if model_config_overrides is not None and len(model_config_overrides) != 0:
            ...
            if hasattr(train_cfg, "save_lora_only") and train_cfg.save_lora_only is True:
                ...
                model = model_class.load_lora(model_path, pretrained_base_path=self._pretrained_base_path)
            else:
            ...
            if hasattr(train_cfg, "save_lora_only") and train_cfg.save_lora_only is True:
                cls = getattr(importlib.import_module(_replace_groot_prefix(cls_module)), cls_name)
                model = cls.load_lora(model_path, pretrained_base_path=self._pretrained_base_path)
            else:
                print(f"Loading model from pretrained directly")
                cls = getattr(importlib.import_module(_replace_groot_prefix(cls_module)), cls_name)
        ...
        # # Load prompt embedding cache before post_initialize so the text
        # # encoder can be skipped during GPU transfer and torch.compile.
        # if prompt_cache_path is not None and os.path.exists(prompt_cache_path):
        #     model.action_head.load_prompt_cache(prompt_cache_path)
