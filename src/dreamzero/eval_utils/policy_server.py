...
class WebsocketPolicyServer:
    ...
    async def run(self):
        async with websockets.asyncio.server.serve(
            ...
            compression=None,
            max_size=None,
            # Disable server-side keepalive pings: policy.infer() blocks the
            # asyncio event loop during (potentially long) GPU inference, so the
            # keepalive task cannot run and would close the connection with
            # 1011 keepalive ping timeout. The client sets its own ping policy.
            ping_interval=None,
        ) as server:
